...
async fn main() -> Result<()> {
    ...
    let checked_updates = if startup_self_update_enabled(&cli) {
        maybe_self_update(&cli).await?
    } else {
        false
    };
...
async fn maybe_self_update(cli: &Cli) -> Result<bool> {
    ...
    let Some(install_dir) = bundle_install_dir(&exe) else {
        return Ok(false);
    };
    let Some(asset_name) = stable_release_asset_name() else {
        return Ok(false);
    };
...
fn startup_self_update_enabled(cli: &Cli) -> bool {
    if !should_attempt_self_update(cli) {
        return false;
    }
...
fn bundle_install_dir(exe: &Path) -> Option<PathBuf> {
    ...
    if dir.join("rpc-server").is_file() && dir.join("llama-server").is_file() {
...
async fn install_latest_bundle(install_dir: &Path, asset_name: &str) -> Result<()> {
    let unique = format!(
        "{}-{}",
        std::process::id(),
    ...
    let result = async {
        ...
        let staged_files = collect_bundle_files(&extracted)?;
...
fn collect_bundle_files(extracted: &Path) -> Result<Vec<String>> {
    ...
    for required in ["mesh-llm", "rpc-server", "llama-server"] {
        anyhow::ensure!(
            files.iter().any(|name| name == required),
            "Downloaded bundle missing {required}"
        );
    }
...
mod tests {
    ...
    #[test]
    fn test_bundle_install_dir_requires_sibling_binaries() {
        let dir = temp_dir("bundle-install");
        let exe = dir.join("mesh-llm");
        std::fs::write(&exe, b"binary").unwrap();
        std::fs::write(dir.join("rpc-server"), b"rpc").unwrap();
        std::fs::write(dir.join("llama-server"), b"llama").unwrap();

        assert_eq!(bundle_install_dir(&exe), Some(dir.clone()));

        let missing = temp_dir("bundle-missing");
        let missing_exe = missing.join("mesh-llm");
        std::fs::write(&missing_exe, b"binary").unwrap();
        assert_eq!(bundle_install_dir(&missing_exe), None);
